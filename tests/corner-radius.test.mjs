...
test('shared corner scale reduces panels and cards without changing round controls', async () => {
  ...
  assert.match(foundation, /--radius-large:\s*12px/u);
  assert.match(foundation, /--radius:\s*10px/u);
  assert.match(foundation, /--radius-small:\s*8px/u);
  assert.match(foundation, /--radius-card:\s*8px/u);
