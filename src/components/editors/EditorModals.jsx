...
const mediaTypeOptions = [
  { value: 'CD', label: 'CD', icon: '/assets/icons/media-type/compact-disc.svg' },
  { value: 'Digital Media', label: 'Digital', icon: '/assets/icons/media-type/file-waveform.svg' },
  { value: 'Vinyl', label: 'Vinyl', icon: '/assets/icons/media-type/record-vinyl.svg' },
  { value: 'Cassette Tape', label: 'Cassette', icon: '/assets/icons/media-type/cassette-tape.svg' },
];
...
export function AlbumTagEditorModal({
  ...
  const [busy, setBusy] = useState({ search: false, save: false, reset: false, apply: false, delete: false });
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        coverDataUrl: form.coverDataUrl,
  ...
  const handleDelete = async () => {
    ...
    setStatusMessage('Deleting album...');
    try {
      await onDelete();
    } catch (error) {
      setStatusMessage(error?.message || 'Unable to delete album.');
    } finally {
  ...
  return (
    <EditorModal
      ...
      onSave={handleSave}
      resetDisabled={busy.reset || busy.save || busy.apply || busy.delete}
      deleteDisabled={busy.delete || busy.reset || busy.save || busy.apply}
      saveDisabled={busy.save || busy.reset || busy.apply || busy.delete}
    >
      <div className={editorBodyClassName}>
        <form className={editorFormClassName} onSubmit={(event) => event.preventDefault()}>
          <div className={coverFrameClassName}>
            <CoverImage
              ...
              src={form.coverDataUrl || form.coverUrl}
