...
export function CoverCaseStack({ covers = [] }) {
...
export function LoginView({
  ...
  const errorMessage = submitError || getErrorMessage(errorCode);
  ...
  return (
    <section className="login-view-shell tw-grid tw-min-h-screen tw-place-items-center tw-px-5 tw-py-8 sm:tw-px-8">
      {canClose ? (
        <button
          className="login-close-button btn-icon"
          type="button"
      ...
      <div className="login-glass-card tw-relative tw-w-full tw-max-w-[540px] tw-overflow-hidden tw-rounded-[38px] tw-border tw-border-[color-mix(in_srgb,var(--text)_12%,transparent)] tw-shadow-[0_38px_120px_rgba(0,0,0,0.42)]">
        ...
        <div className="tw-relative tw-grid tw-gap-6 tw-px-7 tw-py-8 sm:tw-gap-7 sm:tw-px-10 sm:tw-py-10">
          ...
          {isSignedInUser ? (
            <div className="tw-grid tw-gap-3 tw-rounded-[24px] tw-border tw-border-[color-mix(in_srgb,var(--text)_10%,transparent)] tw-bg-[color-mix(in_srgb,var(--glass)_80%,transparent)] tw-p-4">
              ...
              <div className="tw-flex tw-flex-wrap tw-justify-center tw-gap-3">
                {canClose ? (
                  <button className="secondary-button tw-min-w-[150px] tw-justify-center" type="button" onClick={() => onClose?.()}>
                    Open app
                  </button>
                ) : (
                  <a className="secondary-button tw-min-w-[150px] tw-justify-center" href={nextPath || '/'}>
                    Open app
                  </a>
                )}
                <button className="secondary-button tw-min-w-[150px] tw-justify-center" type="button" data-logout-button="true">
                  Logout
                </button>
              </div>
            </div>
          ...
          {errorMessage ? (
            <div className="tw-rounded-[22px] tw-border tw-border-[color-mix(in_srgb,var(--favorite-red)_56%,transparent)] tw-bg-[color-mix(in_srgb,var(--favorite-red)_14%,transparent)] tw-p-4 tw-text-[0.95rem] tw-font-bold tw-text-text" role="alert">
              {errorMessage}
            </div>
          ) : null}

          <form className="tw-grid tw-gap-4" method="post" action="/login" onSubmit={handleSubmit} aria-busy={submitting}>
