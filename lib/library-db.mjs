...
const COLLECTION_MEMBERSHIP_VERSION = '1';
...
export async function addPlaylistTrack(databasePath, ownerKey, playlistId, trackId) {
  const result = await addPlaylistTracks(databasePath, ownerKey, playlistId, [trackId]);
  return {
    ...result,
...
export async function addPlaylistTracks(databasePath, ownerKey, playlistId, trackIds) {
  const cleanTrackIds = [...new Set(
    (Array.isArray(trackIds) ? trackIds : [])
      .map((trackId) => String(trackId || '').trim())
      .filter(Boolean),
  )];
  if (!databasePath || !ownerKey || !playlistId || cleanTrackIds.length === 0) {
  ...
  try {
    ...
    const placeholders = cleanTrackIds.map(() => '?').join(', ');
    const availableTrackIds = new Set(db.prepare(`
      SELECT id FROM tracks WHERE id IN (${placeholders})
    `).all(...cleanTrackIds).map((row) => row.id));
    if (cleanTrackIds.some((trackId) => !availableTrackIds.has(trackId))) {
      throw createPlaylistDatabaseError('TRACK_NOT_FOUND', 'Track not found.');
    }

    const existingTrackIds = new Set(db.prepare(`
    ...
    db.exec('BEGIN');
    try {
      for (const trackId of cleanTrackIds) {
        if (existingTrackIds.has(trackId)) continue;
        insertTrack.run(playlistId, trackId, nextPosition, now);
        nextPosition += 1;
        addedCount += 1;
      }
      if (addedCount > 0) {
    ...
    return {
      addedCount,
      skippedCount: cleanTrackIds.length - addedCount,
...
export async function readLibraryAlbumPage(databasePath, options = {}) {
  ...
  try {
    ...
    const whereSql = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
    const knownTotal = normalizeKnownPageTotal(options.knownTotal);
    const totalAlbums = knownTotal
      ?? (db.prepare(`SELECT COUNT(*) AS count FROM albums ${whereSql}`).get(params).count || 0);
    const rows = db.prepare(`
      SELECT * FROM albums
      ${whereSql}
...
export async function readCollectionFolders(databasePath, options = {}) {
  ...
  try {
    const search = cleanSearchValue(options.search);
    const params = {};
    const where = ["album_collections.collection_name != ''"];
    addTokenSearchFilter(where, params, ['album_collections.collection_name'], search, 'collectionNameSearch');
    addAlbumFolderFilter(where, params, options.folders, 'collectionList');

    const rows = db.prepare(`
      WITH matched_collections AS (
        SELECT
          album_collections.collection_name,
          LOWER(album_collections.collection_name) AS collection_key,
          albums.id AS album_id,
          albums.cover_track_id,
          albums.artist,
          albums.title
        FROM album_collections
        INNER JOIN albums ON albums.id = album_collections.album_id
        WHERE ${where.join(' AND ')}
      )
      SELECT
        (
          SELECT first_collection.collection_name
          FROM matched_collections AS first_collection
          WHERE first_collection.collection_key = matched_collections.collection_key
          ORDER BY first_collection.collection_name COLLATE NOCASE
          LIMIT 1
        ) AS collection_name,
        COUNT(DISTINCT matched_collections.album_id) AS album_count,
        GROUP_CONCAT(matched_collections.album_id, char(31)) AS album_ids,
        (
          SELECT first_album.album_id
          FROM matched_collections AS first_album
          WHERE first_album.collection_key = matched_collections.collection_key
          ORDER BY first_album.artist, first_album.title, first_album.album_id
          LIMIT 1
        ) AS first_album_id,
        (
          SELECT first_album.cover_track_id
          FROM matched_collections AS first_album
          WHERE first_album.collection_key = matched_collections.collection_key
          ORDER BY first_album.artist, first_album.title, first_album.album_id
          LIMIT 1
        ) AS first_album_cover_track_id,
        (
          SELECT cover_collection.cover_track_id
          FROM matched_collections AS cover_collection
          WHERE cover_collection.collection_key = matched_collections.collection_key
            AND cover_collection.cover_track_id != ''
          ORDER BY cover_collection.artist, cover_collection.title
          LIMIT 1
        ) AS cover_track_id
      FROM matched_collections
      GROUP BY matched_collections.collection_key
    ...
    return {
      ...
      folders: rows.map((row) => ({
        ...
        firstAlbumCoverTrackId: row.first_album_cover_track_id || '',
...
async function updateCollectionName(databasePath, fromName, toName) {
  ...
  try {
    ...
    `).all();
    const matches = rows.filter((row) => parseCollectionNames(row.collection_name)
      .some((collectionName) => normalizeCollectionKey(collectionName) === sourceKey));
    if (matches.length === 0) {
      return { updated: 0 };
    }
    ...
    try {
      for (const item of matches) {
        updateAlbum.run(replaceCollectionName(item.collection_name, fromName, cleanToName), item.id);
      }
      rebuildAlbumCollectionMemberships(db);
      db.exec('COMMIT');
    } catch (error) {
      db.exec('ROLLBACK');
      throw error;
...
export async function readCollectionFolderAlbumPage(databasePath, folderPath, options = {}) {
  ...
  try {
    ...
    where[0] = `EXISTS (
      SELECT 1
      FROM album_collections
      WHERE album_collections.album_id = albums.id
        AND album_collections.collection_name = $collectionName COLLATE NOCASE
    )`;
...
export async function readRecentlyAddedAlbumPage(databasePath, options = {}) {
  if (!databasePath || !existsSync(databasePath)) {
    return createEmptyLibrary();
  }
  ...
  try {
    ...
    if (albumIds.length > 0) {
      where.push(`albums.id IN (${albumIds.map((_, index) => `$recentAlbumId${index}`).join(', ')})`);
      albumIds.forEach((albumId, index) => {
        params[`$recentAlbumId${index}`] = albumId;
      });
    }

    if (excludeAlbumIds.length > 0) {
      where.push(`albums.id NOT IN (${excludeAlbumIds.map((_, index) => `$recentExcludeAlbumId${index}`).join(', ')})`);
      excludeAlbumIds.forEach((albumId, index) => {
        params[`$recentExcludeAlbumId${index}`] = albumId;
      });
    }

    addAlbumFolderFilter(where, params, options.folders, 'recent');
    const whereSql = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
    const rows = db.prepare(`
      SELECT albums.*
      FROM albums
      ${whereSql}
      ORDER BY albums.latest_mtime_ms DESC, albums.artist, albums.title
      LIMIT $limit
    `).all({ ...params, $limit: limit });
    const albums = attachCoverTracksForCardPage(db, rows.map(rowToAlbum), options);
    const tracks = shouldIncludePageTracks(options)
...
export async function readTrackPage(databasePath, options = {}) {
  ...
  try {
    ...
    const whereSql = where.length > 0 ? `WHERE ${where.join(' AND ')}` : '';
    const knownTotal = normalizeKnownPageTotal(options.knownTotal);
    const totalTracks = knownTotal
      ?? (db.prepare(`SELECT COUNT(*) AS count FROM tracks ${whereSql}`).get(params).count || 0);
...
export async function readArtistPage(databasePath, options = {}) {
  ...
  try {
    ...
    const total = knownTotal
      ?? (db.prepare(`SELECT COUNT(*) AS count FROM (SELECT artist FROM tracks ${artistWhereSql} GROUP BY artist)`).get(params).count || 0);
...
export async function writeLibraryDatabase(databasePath, library) {
  ...
  const db = await openDatabase(databasePath);
  const rebuildSearchIndexes = isSearchIndexAvailable(db);
  let writePhase = 'starting transaction';
  try {
    db.exec('BEGIN IMMEDIATE');
    if (rebuildSearchIndexes) {
      writePhase = 'suspending search index triggers';
      dropSearchIndexTriggers(db);
    }
    writePhase = 'clearing old library rows';
    ...
    writePhase = 'writing library metadata';
    ...
    insertMetadata.run('collectionMembershipVersion', COLLECTION_MEMBERSHIP_VERSION);
    ...
    const insertTrack = db.prepare(`
      INSERT INTO tracks (
        ...
        duration, audio_quality_json, replay_gain_json
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);

    writePhase = 'writing tracks';
    for (const track of library.tracks || []) {
      ...
      insertTrack.run(
        ...
        JSON.stringify(track.replayGain || null),
      );
    }
    ...
    `);
    writePhase = 'writing folder index';
    ...
    const insertAlbumCollection = db.prepare('INSERT INTO album_collections (album_id, collection_name, position) VALUES (?, ?, ?)');

    writePhase = 'writing albums';
    for (const album of library.albums || []) {
      const collectionNames = getAlbumCollectionNames(album);
      insertAlbum.run(
        album.id,
        album.title || '',
        album.artist || '',
        ...
        getAlbumLatestMtimeMs(album, library.tracks || []),
      );
      ...
      for (const [index, collectionName] of collectionNames.entries()) {
        insertAlbumCollection.run(album.id, collectionName, index);
      }
    }

    if (rebuildSearchIndexes) {
      writePhase = 'rebuilding search indexes';
      rebuildSearchIndexRows(db);
      createSearchIndexTriggers(db);
      setMetadata(db, 'searchIndexVersion', SEARCH_INDEX_VERSION);
    }
    writePhase = 'committing library database';
    db.exec('COMMIT');
  } catch (error) {
    try {
      db.exec('ROLLBACK');
    ...
    throw createLibraryDatabaseWriteError(error, writePhase);
  } finally {
    db.close();
  }
}

function createLibraryDatabaseWriteError(error, phase) {
  const message = error instanceof Error ? error.message : String(error || 'Unknown SQLite error');
  const contextualError = new Error(`Library database write failed while ${phase}: ${message}`, { cause: error });
  for (const key of ['code', 'errcode', 'errstr']) {
    if (error && Object.prototype.hasOwnProperty.call(error, key)) {
      contextualError[key] = error[key];
    }
  }
  return contextualError;
}

export async function updateTrackCoverCacheRecords(databasePath, tracks = []) {
  if (!databasePath || !Array.isArray(tracks) || tracks.length === 0) return 0;

  const coverTracks = tracks.filter((track) => track?.id && track.cachedCoverPath);
  if (coverTracks.length === 0) return 0;
  ...
  try {
    ...
    const updateTrack = db.prepare(`
      UPDATE tracks
      SET cached_cover_path = ?, cached_cover_format = ?, has_embedded_cover = ?
      WHERE id = ?
    `);
    let changes = 0;
    for (const track of coverTracks) {
      changes += updateTrack.run(
        track.cachedCoverPath || '',
        track.cachedCoverFormat || '',
        track.hasEmbeddedCover ? 1 : 0,
        track.id,
...
async function openDatabase(databasePath) {
  ...
  // Version 6 already has the data/search indexes; upgrade only the browse indexes.
  if (databaseVersion === 6) {
    try {
      db.exec('BEGIN IMMEDIATE');
      db.exec('DROP INDEX IF EXISTS idx_tracks_folder_path');
      db.exec('DROP INDEX IF EXISTS idx_album_tracks_track');
      db.exec('CREATE INDEX idx_tracks_folder_path ON tracks(folder_path, id)');
      db.exec('CREATE INDEX idx_album_tracks_track ON album_tracks(track_id, album_id)');
  ...
  try {
    ...
    CREATE TABLE IF NOT EXISTS tracks (
      ...
      track_number_label TEXT NOT NULL DEFAULT '',
      disc_number INTEGER,
      date TEXT,
      year TEXT,
    ...
    CREATE TABLE IF NOT EXISTS albums (
      ...
      audio_quality_json TEXT,
      latest_mtime_ms INTEGER NOT NULL DEFAULT 0
    );
    ...
    DROP INDEX IF EXISTS idx_album_tracks_track;
    CREATE INDEX idx_album_tracks_track ON album_tracks(track_id, album_id);
    ...
    CREATE INDEX IF NOT EXISTS idx_album_collections_name
      ON album_collections(collection_name COLLATE NOCASE, position, album_id);
  ...
  `);
    ...
    ensureColumn(db, 'tracks', 'track_number_label', "TEXT NOT NULL DEFAULT ''");
    ensureColumn(db, 'tracks', 'replay_gain_json', 'TEXT');
    ensureColumn(db, 'albums', 'collection_name', "TEXT NOT NULL DEFAULT ''");
    ensureColumn(db, 'albums', 'latest_mtime_ms', "INTEGER NOT NULL DEFAULT 0");
    db.exec(`
      DROP INDEX IF EXISTS idx_tracks_folder_path;
      CREATE INDEX idx_tracks_folder_path ON tracks(folder_path, id);
      ...
      CREATE INDEX IF NOT EXISTS idx_albums_recently_added
        ON albums(latest_mtime_ms DESC, artist, title, id);
    `);
    migrateFolderIndex(db);
    migrateAlbumIndex(db);
    migrateAlbumCollectionNames(db);
    migrateAlbumCollectionMemberships(db);
    migrateAlbumLatestMtime(db);
...
function dropSearchIndexTriggers(db) {
  db.exec(`
...
function createSearchIndexTriggers(db) {
  db.exec(`
...
function rebuildSearchIndexRows(db) {
  db.exec(`
...
function migrateAlbumCollectionMemberships(db) {
  if (getMetadata(db, 'collectionMembershipVersion') === COLLECTION_MEMBERSHIP_VERSION) return;
  db.exec('BEGIN IMMEDIATE');
  try {
    rebuildAlbumCollectionMemberships(db);
    db.exec('COMMIT');
...
function rebuildAlbumCollectionMemberships(db) {
  db.prepare('DELETE FROM album_collections').run();
  const insertAlbumCollection = db.prepare('INSERT INTO album_collections (album_id, collection_name, position) VALUES (?, ?, ?)');
  const rows = db.prepare('SELECT id, collection_name FROM albums ORDER BY artist, title, id').all();
  for (const row of rows) {
    for (const [index, collectionName] of parseCollectionNames(row.collection_name).entries()) {
      insertAlbumCollection.run(row.id, collectionName, index);
    }
  }
  setMetadata(db, 'collectionMembershipVersion', COLLECTION_MEMBERSHIP_VERSION);
}

function migrateAlbumLatestMtime(db) {
  const missingLatestMtimeCount = db.prepare('SELECT COUNT(*) AS count FROM albums WHERE latest_mtime_ms = 0').get().count || 0;
  if (missingLatestMtimeCount === 0) return;

  db.exec(`
    UPDATE albums
    SET latest_mtime_ms = COALESCE((
      SELECT MAX(COALESCE(tracks.mtime_ms, 0))
      FROM album_tracks
      INNER JOIN tracks ON tracks.id = album_tracks.track_id
      WHERE album_tracks.album_id = albums.id
    ), 0)
...
function rebuildAlbumIndex(db) {
  ...
  for (const row of tracks) {
    ...
    album.latestMtimeMs = Math.max(album.latestMtimeMs || 0, Number(row.mtime_ms) || 0);
  ...
  for (const album of albumMap.values()) {
    ...
    insertAlbum.run(
      ...
      numberOrNull(album.latestMtimeMs) || 0,
    );
    for (const [index, trackId] of album.trackIds.entries()) {
      insertAlbumTrack.run(album.id, trackId, index);
    }
...
function getAlbumLatestMtimeMs(album, tracks = []) {
  const trackIds = new Set(Array.isArray(album?.trackIds) ? album.trackIds : []);
  if (trackIds.size === 0) return 0;
  let latestMtimeMs = 0;
  for (const track of tracks) {
    if (trackIds.has(track.id)) {
      latestMtimeMs = Math.max(latestMtimeMs, Number(track.mtimeMs) || 0);
    }
  }
  return Math.round(latestMtimeMs);
}
...
function getAlbumCollectionNames(album) {
  const fromArray = Array.isArray(album.collectionNames)
    ? album.collectionNames.flatMap((collectionName) => parseCollectionNames(collectionName))
    : [];
  const collectionNames = fromArray.length > 0 ? fromArray : parseCollectionNames(album.collectionName);
...
function rowToTrack(row) {
  return {
    ...
    replayGain: parseJson(row.replay_gain_json, null),
  };
}
...
function rowToAlbum(row) {
  ...
  const trackIds = parseJson(row.track_ids_json, []);
  return {
    id: row.id,
    title: row.title,
    artist: row.artist,
    ...
    year: row.year || null,
    collectionName,
    collectionNames: parseCollectionNames(row.collection_names || collectionName),
    coverTrackId: row.cover_track_id || null,
    trackIds,
    trackCount: trackIds.length,
    audioQuality: parseJson(row.audio_quality_json, null),
    latestMtimeMs: Number(row.latest_mtime_ms) || 0,
  };
}
...
function normalizeKnownPageTotal(value) {
...
function addAlbumFolderFilter(where, params, folders, prefix = 'folder') {
  ...
  const folderQueries = normalizedFolders.flatMap((folderPath, index) => {
    const exactKey = `$${prefix}Folder${index}`;
    const descendantStartKey = `$${prefix}FolderDescendantStart${index}`;
    const descendantEndKey = `$${prefix}FolderDescendantEnd${index}`;
    params[exactKey] = folderPath;
    params[descendantStartKey] = `${folderPath}/`;
    params[descendantEndKey] = `${folderPath}0`;
    return [
      `SELECT album_tracks.album_id
       FROM tracks INDEXED BY idx_tracks_folder_path
       INNER JOIN album_tracks ON album_tracks.track_id = tracks.id
       WHERE tracks.folder_path = ${exactKey}`,
      `SELECT album_tracks.album_id
       FROM tracks INDEXED BY idx_tracks_folder_path
       INNER JOIN album_tracks ON album_tracks.track_id = tracks.id
       WHERE tracks.folder_path >= ${descendantStartKey}
         AND tracks.folder_path < ${descendantEndKey}`,
    ];
  });
  where.push(`albums.id IN (
    ${folderQueries.join('\n    UNION\n    ')}
  )`);
}
