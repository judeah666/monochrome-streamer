  ...
  for (const root of roots) {
    await walkDirectory(root, async (filePath) => {
      ...
      tracks.push({
        ...
        replayGain: tagMetadata.replayGain,
...
async function readTaggedMetadata(filePath, metadataParser, options = {}) {
  ...
  try {
    ...
    return {
      ...
      replayGain: createReplayGainMetadata(parsed.common),
      hasEmbeddedCover: false,
    };
  } catch {
    return {
...
export function createReplayGainMetadata(common = {}) {
  const replayGain = {
    trackGainDb: readReplayGainDb(common.replaygain_track_gain),
    trackPeak: readReplayGainPeak(common.replaygain_track_peak),
    albumGainDb: readReplayGainDb(common.replaygain_album_gain),
    albumPeak: readReplayGainPeak(common.replaygain_album_peak),
  };

  return Object.values(replayGain).some((value) => value != null) ? replayGain : null;
}

function readReplayGainDb(value) {
  const candidate = value && typeof value === 'object' ? value.dB : value;
  if (candidate == null || candidate === '') return null;
  const number = typeof candidate === 'string' ? Number.parseFloat(candidate) : Number(candidate);
  return Number.isFinite(number) ? number : null;
}

function readReplayGainPeak(value) {
...
export function mergeTargetedLibraryScan(cachedLibrary, scannedLibrary, options = {}) {
  const selectedFolders = new Set(normalizeFolderNames(options.selectedFolders));
  const scannedFolders = new Set(normalizeFolderNames(options.scannedFolders));
  const cachedTracks = Array.isArray(cachedLibrary?.tracks) ? cachedLibrary.tracks : [];
  const scannedTracks = Array.isArray(scannedLibrary?.tracks) ? scannedLibrary.tracks : [];

  const retainedTracks = cachedTracks.filter((track) => {
    const folder = getTopLevelFolder(track.relativePath || '');
    return selectedFolders.has(folder) && !scannedFolders.has(folder);
  });
  const replacementTracks = scannedTracks.filter((track) => {
    const folder = getTopLevelFolder(track.relativePath || '');
    return selectedFolders.has(folder) && scannedFolders.has(folder);
  });
  const tracks = [...retainedTracks, ...replacementTracks].sort(compareTracks);
  const albums = buildAlbums(tracks, {
  ...
  return {
    generatedAt: scannedLibrary?.generatedAt || new Date().toISOString(),
    trackCount: tracks.length,
    albumCount: albums.length,
    tracks,
    albums,
...
function normalizeFolderNames(folders) {
  return [...new Set((Array.isArray(folders) ? folders : [])
    .map((folder) => String(folder || '').replaceAll('\\', '/').replace(/^\/+|\/+$/gu, '').trim())
    .filter(Boolean))];
}
