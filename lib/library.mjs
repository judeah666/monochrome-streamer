...
export async function scanMusicLibrary(libraryPath, options = {}) {
  ...
  const totalFiles = await countAudioFilesInRoots(roots);
  ...
  for (const root of roots) {
    await walkDirectory(root, async (filePath) => {
      ...
      if (canReuseCachedTrack(cachedTrack, stats, { scanMetadata, scanDurations })) {
        tracks.push({
          ...cachedTrack,
          path: filePath,
        });
  ...
  for (const track of tracks) {
    ...
    if (!albumMap.has(albumKey)) {
      albumMap.set(albumKey, {
        ...
        collectionName: inferCollectionNameFromFolderPath(trackFolderPath, track.album),
        coverTrackId: track.coverArtPath || track.hasEmbeddedCover ? track.id : null,
        trackIds: [],
        folderPaths: [],
      });
    }
    ...
    if (!album.collectionName) {
      album.collectionName = inferCollectionNameFromFolderPath(trackFolderPath, track.album);
    }
...
export function inferCollectionNameFromFolderPath(folderPath, albumTitle = '') {
  ...
  if (parentCollectionSegment) {
    return humanizeName(parentCollectionSegment);
  }
  ...
  if (/\bcollections?\b/iu.test(joinedPath) && /\balbums?\b/iu.test(joinedPath)) {
    ...
    return collectionSegment ? humanizeName(collectionSegment) : '';
  }
